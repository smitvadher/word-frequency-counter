namespace Counter.Core.Tests
{
    public class WordCounterTests
    {
        ...
        [Fact]
        public void CountWordFrequencies_NullContent_WithDictionary_ReturnsSameDictionary()
        {
            // Arrange
            var wordCounter = new WordCounter();
            var fd = new Dictionary<string, int>
            {
                { "Test", 2 },
            };

            // Act
            var result = wordCounter.CountWordFrequencies(null, fd);
        ...
        public void CountWordFrequencies_ValidContent_ReturnsWordFrequencies(string content, IDictionary<string, int> expectedResult)
        {
            ...
            // Act
            var result = wordCounter.CountWordFrequencies(content, null);
            ...
            IDictionary<string, int> expectedFrequencyDictionary)
        {
            ...
            // Act
            var result = wordCounter.CountWordFrequenciesFromChunk(chunk, frequencyDictionary, previousRemainingWord, out var remainingWord);

            // Assert
            Assert.NotNull(result);
            Assert.Equal(expectedRemainingWord, remainingWord);
            Assert.Equal(expectedFrequencyDictionary, result);
        }
